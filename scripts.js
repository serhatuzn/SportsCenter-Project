let btn = document.querySelector(".button-container"); // bütün utonları içeren containerlar'ı seç
...
btn.addEventListener("click", (e) => {
    ...
    // Tıklanan butonun id'sine göre ilgili fonksiyonu çağırır ve o id'li fonksiyon işlevini gerçekleştirir
    switch(e.target.id) {
        case "yoga":
            yoga(); // Yoga özelliklerini günceller
            break;
        case "group":
            group(); // Grup özellikini günceller
            break;
        case "solo":
            solo(); // Solo özellikini günceller
            break;
        case "stretch":
...
// OUR CLASSES da Yoga bilgilerini güncelleyen fonksiyon
function yoga() {
    document.querySelectorAll("#features-container h6")[0].innerHTML = "Why Are Your Yoga?"; // İlk olarak başlığı günceller
    ...
    repellendus architecto ducimus explicabo reiciendis alias quasi culpa libero amet similique nisi!`; // sonra bulunan ilk paragrafı günceller
...
function changeBg() {
    let navbar = document.querySelector(".navbar-container"); // Navbar'ı seçiyoruz
    let scrollValue = window.scrollY;
    if(scrollValue > 100) {
        navbar.classList.add('bgColor'); // 100'ü geçince arka plan rengini değiştirir
    } else {
        navbar.classList.remove('bgColor'); // geri çekildiğinde renk eski haline döndürür
    }
}
